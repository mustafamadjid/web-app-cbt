...
import (
	...
	fake_test "github.com/mustafamadjid/web-app-cbt/internal/core/service/kelas/create/fake_test"
...
func TestCreateKelasService_CreateTingkatKelas(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			repo:       &fake_test.FakeKelasRepo{ExistTingkatKelasRet: true},
			wantErr:    coreerror.ErrTingkatKelasExist,
			wantCreate: false,
		},
		{
			name:       "Branch 2 -> gagal cek tingkat kelas",
			repo:       &fake_test.FakeKelasRepo{ExistTingkatKelasErr: existErr},
			wantErr:    existErr,
			wantCreate: false,
		},
		{
			name:       "Branch 3 -> gagal create tingkat kelas",
			repo:       &fake_test.FakeKelasRepo{CreateTingkatKelasErr: createErr},
			wantErr:    createErr,
			wantCreate: true,
		},
		{
...
func TestCreateKelasService_CreateNamaKelas(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			repo:               &fake_test.FakeKelasRepo{ExistNamaKelasRet: true},
		...
		},
		{
			...
			repo:               &fake_test.FakeKelasRepo{ExistNamaKelasErr: existErr},
			wantErr:            existErr,
			wantCreate:         false,
			wantExistNamaKelas: trimmedNamaKelas,
		},
		{
			...
			repo:               &fake_test.FakeKelasRepo{CreateNamaKelasErr: createErr},
			wantErr:            createErr,
			wantCreate:         true,
			wantExistNamaKelas: trimmedNamaKelas,
		},
