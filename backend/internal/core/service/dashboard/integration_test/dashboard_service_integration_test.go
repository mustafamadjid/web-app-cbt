...
import (
	...
	dashboardrepo "github.com/mustafamadjid/web-app-cbt/internal/adapter/repository/postgres/dashboard"
...
func TestDashboardService_GetDashboardStatistik(t *testing.T) {
	ctx, tx := testutil.BeginRollbackTx(t)
	fixtures := testutil.NewFixtures(t, ctx, tx)

	repo := dashboardrepo.NewDashboardStatistikRepo(tx, nil)
	svc := dashboard_service.NewDashboardService(repo)

	before, err := svc.GetDashboardStatistik(ctx)
	...
	exam := fixtures.CreateUjian(bank.ID, kelas.ID, testutil.Ptr(namaKelas.ID), guru.ID, "Dashboard Ujian")
	base := time.Date(2000, time.January, 1, 9, 0, 0, 0, time.UTC)
	fixtures.CreateJadwalUjian(exam.ID, sesi.ID, ruang.ID, pengawas.ID, base, base, base.Add(time.Hour), ujian.SELESAI)

	after, err := svc.GetDashboardStatistik(ctx)
	require.NoError(t, err)
