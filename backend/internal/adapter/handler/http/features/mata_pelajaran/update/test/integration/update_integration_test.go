...
func TestUpdateMapelIntegration(t *testing.T) {
	...
	kode := fmt.Sprintf("MPUPD%d", time.Now().UnixNano()%1_000_000_000)
	nama := "Mapel Update " + repotestutil.UniqueSuffix("api")
	rec := handlertestutil.DoJSON(t, testApp.HTTP.Handler, http.MethodPost, "/admin/mata-pelajaran", map[string]any{
		"id_kelas": kelas.ID, "kode_mapel": kode, "nama_mapel": nama, "deskripsi": "Deskripsi awal",
	}, adminCookie)
	...
	t.Run("updates mapel successfully", func(t *testing.T) {
		updatedName := nama + " Updated"
		rec := handlertestutil.DoJSON(t, testApp.HTTP.Handler, http.MethodPatch, fmt.Sprintf("/admin/mata-pelajaran/%d", idMapel), map[string]any{
			"nama_mapel": updatedName, "deskripsi": "Deskripsi update",
		}, adminCookie)
		...
		assert.Equal(t, updatedName, item["nama_mapel"])
	})
}
