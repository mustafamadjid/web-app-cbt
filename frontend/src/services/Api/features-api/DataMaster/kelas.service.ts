...
export async function getTingkatKelasOptions(): Promise<number[]> {
  await sleep(150);
  return Array.from(new Set(DUMMY_KELAS.map((kelas) => kelas.tingkat_kelas))).sort(
    (a, b) => a - b
  );
}
...
export async function getKelas(
  ...
  const filtered = DUMMY_KELAS.filter((kelas) => {
    if (params.tingkatKelas && kelas.tingkat_kelas !== params.tingkatKelas) {
      return false;
    }
