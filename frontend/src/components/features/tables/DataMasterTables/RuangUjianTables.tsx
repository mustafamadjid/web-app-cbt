...
export const RuangUjianTables: React.FC = () => {
  ...
  const semuaTerlihatTerpilih =
    ...
    daftarRuangUjian.every((r) => idTerpilih.has(r.id));

  const togglePilihSemuaTerlihat = () => {
    setIdTerpilih((prev) => {
      const next = new Set(prev);
      if (semuaTerlihatTerpilih) {
        daftarRuangUjian.forEach((r) => next.delete(r.id));
      } else {
        daftarRuangUjian.forEach((r) => next.add(r.id));
      }
      return next;
    });
  };
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="text-sm text-slate-600">
          {loading ? (
            ...
            <span>
              Menampilkan <span className="font-medium">{daftarRuangUjian.length}</span>{" "}
              hasil.
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-200 bg-white px-4 py-3 sm:px-6">
          <div className="flex flex-1 items-center justify-between">
            <p className="text-sm text-slate-700">
              ...
              <span className="font-medium">{daftarRuangUjian.length}</span> dari{" "}
              <span className="font-medium">{daftarRuangUjian.length}</span> hasil
            </p>
